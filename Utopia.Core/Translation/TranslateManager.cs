...
public class TranslateManager : SafeDictionary<Guuid, ITranslateProvider>, ITranslateManager
{
    ...
    private TranslateIdentifence _current = new();

    public TranslateIdentifence CurrentLanguage
    {
        get
        {
            lock (_lock)
    ...
    public bool Contains(TranslateIdentifence language, Guuid? translateProviderId, string item) => TryGetTranslate(language, translateProviderId, item, out _);

    public bool TryGetTranslate(TranslateIdentifence language, Guuid? translateProviderId, string item, out string? result)
    {
        ...
        lock (_lock)
        {
            ...
            {
                if (value!.TryGetItem(language, item, out result))
                {
                    return true;
                }
                else
            ...
            else if (translateProviderId is null)
            {
                ...
                {
                    if (provider.Value.TryGetItem(language, item, out result))
                    {
                        return true;
                    }
                }
